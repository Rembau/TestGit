...
public class Test_3 extends JFrame
{
	...
	public Test_3()
	{
		...
		c= new Circle(100,100,450,350);
		c.init();
		cs.add(c);
		c= new Circle(100,100,550,310);
		c.init();
		cs.add(c);
		
		c= new Circle(100,100,600,100);
		c.init();
		cs.add(c);
	...
	class Panel_ extends /*JPanel*/JComponent
	{
		private static final long serialVersionUID = 1L;
		int mark=0;
		public void paint(Graphics g)
		{	
			...
			g.drawLine(light.x, light.y, light.x, light.rooty);
			g.drawLine(0, light.rooty, this.getWidth(), light.rooty);

			g.drawLine(0, this.getHeight()*3/4, this.getWidth(), this.getHeight()/4);
		}
		public void paint_(Graphics2D g,Circle c)
		{
			c.s.handle();
			g.setColor(Color.gray);
			g.rotate(c.s.theta, c.centerx_, c.centery_+c.h_/2.0);
			...
			g.setColor(Color.red);
			g.fillOval(c.centerx_, c.centery_+c.h_/2, 5, 5);
			g.fillOval(c.s.x, c.s.y, 2, 2);
			c.l.init_();
			for(int i=250;i>0;i--)
			{
				int j=i;
	...
	class Circle
	{
		...
		class Shadow
		{
			...
			public void init()
			{
				double x1=Circle.this.centerx_-light.x;
				double y1=Circle.this.centery_+Circle.this.h_/2-light.rooty;
				double y2=Circle.this.centery_-light.y;
				w_=(int) (Circle.this.w_*Math.sqrt(y2*y2 + x1*x1+y1*y1)/y2);
				//h_=(int) ((1/9.0f)*Circle.this.h_);
				
				double a=0;
				if(y1!=0)
					a = Math.atan(Math.abs(y1/x1));
				h_=(int) (Circle.this.w_*((a+0.2)/(Math.PI/2+0.2)));
				//h_=(int) (0.2*Math.abs(Circle.this.centerx_-light.x)+0.2*(Circle.this.centery_+Circle.this.h_/2-light.rooty));
				double f=10.0;	
				
				double i=Math.sqrt((x1*x1)+f*y1*y1);
				System.out.println(x1);
				if(y1==0 && x1!=0)
					centerx_=(int) (Math.signum(Circle.this.centerx_-light.x)*(Circle.this.w_*i/(1.0*(Circle.this.centery_-light.y)))/2.0+Circle.this.centerx_);
				else if(y1==0)
					centerx_=Circle.this.centerx_;
				else if(x1!=0)
					centerx_=(int) (Circle.this.centerx_+Math.signum(x1)*w_/2);
				else 
					centerx_=Circle.this.centerx_+w_/2;
				System.out.println(centerx_+" "+w_+" "+h_);
				centery_=Circle.this.centery_+Circle.this.h_/2;
				if(x1!=0)
					theta=Math.atan(y1/(1.0*Math.abs(x1)));
				if(theta!=0 && (Circle.this.centerx_-light.x)<0)
				{
					theta=-theta;
				}
			}
